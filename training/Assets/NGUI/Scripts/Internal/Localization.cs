...
public static class Localization
{
    ...
    static public string Get(string langKey, string defaultText)
    {
        if (Localization.Exists(langKey))
        {
            return Localization.Get(langKey, false);
        }
#if UNITY_EDITOR
        else
        {
            if (!string.IsNullOrEmpty(defaultText) && Localization.language != "KOREAN")
            {
                return string.Format(Localization.unknown_value_fmt, defaultText);
            }
        }
#endif
    ...
    static public string Get(string key)
    {
        ...
        {
            string org_key = key.Substring(alias_prefix.Length);
            return _Get(org_key);
        }
    ...
    */
...
#if UNITY_EDITOR
    static public readonly string none_value_fmt = "[00FF00][b]*[/b]{0}[-]";
    static public readonly string unknown_value_fmt = "[FF0000][b]?[/b]{0}[-]";
#endif
    ...
    static public string Get (string key, bool apply_none_value_fmt)
	{
	...
	static void UpdateCSV (BetterList<string> values, List<int> valid_index)
	{
        ...
        else if (valid_index.Count > 1)
        {
            string[] temp = new string[valid_index.Count - 1];
            for (int i = 1; i < valid_index.Count; ++i)
            {
                temp[i - 1] = values[valid_index[i]];
            }

            try
            {
                mDictionary[values[valid_index[0]]] = temp;
            }
            catch (System.Exception ex)
            {
                Debug.LogError("Unable to add '" + values[0] + "' to the Localization dictionary.\n" + ex.Message);
    ...
    static void UpdateCSV_SpecificLanguage(string [] newLanguages, BetterList<string> new_values, string lang_key)
    {
        if (new_values.size < 2) return;
        string[] temp = new string[new_values.size - 1];
        for (int i = 1; i < new_values.size; ++i) temp[i - 1] = new_values[i];

        int cur_index = -1;
        for (int i=0; i<mLanguages.Length; i++)
        {
            if (lang_key == mLanguages[i])
            {
                cur_index = i;
                break;
            }
        ...
        if (cur_index < 0)
        {
            Debug.Log( "Unknown Language : " + lang_key);
            return;
        }
        ...
        if (new_index < 0)
        {
			Debug.Log( "Unknown New Language : " + lang_key);
            return;
        }
        ...
        try
        {
            ...
            string[] curValues;
            if (mDictionary.TryGetValue(new_values[0], out curValues))
            {
                string newValue = new_values[new_index + 1];
                string curValue = curValues[cur_index];

                if (!string.IsNullOrEmpty(newValue))
                {
                    curValues[cur_index] = newValue;
                }
            }
        }
        catch (System.Exception ex)
        {
            Debug.LogError("lang=" + lang_key + ", Unable to add '" + new_values[0] + "' to the Localization dictionary.\n" + ex.Message);
        }
    }

    static void UpdateCSV_ExceptLanguage(BetterList<string> values, string ignore_lang)
    {
        if (values.size < 2) return;
        string[] temp = new string[values.size - 1];
        for (int i = 1; i < values.size; ++i) temp[i - 1] = values[i];
        ...
        try
        {
            ...
            string[] curValues;
            if (!mDictionary.TryGetValue(values[0], out curValues))
            {
                curValues = new string[values.size - 1];

                for (int i = 0; i < curValues.Length; i++)
                {
                    curValues[i] = string.Empty;
                }

                mDictionary.Add(values[0], curValues);
            }

            for (int i=0; i< curValues.Length; i++)
            {
                if (i != ignore_index && !string.IsNullOrEmpty(values[i + 1]))
                {
                    curValues[i] = values[i + 1];
                }
            }
        }
        catch (System.Exception ex)
    ...
    static public void DiscardOtherLanguages(string except_language)
    {
        if (mLanguages==null || mLanguage.Length==0)
        {
            return;
        }
        ...
        for (int i=0; i<mLanguages.Length; i++)
        {
            string lang = mLanguages[i];
            if (except_language == lang)
            {
                keep_index = i;
                break;
            }
