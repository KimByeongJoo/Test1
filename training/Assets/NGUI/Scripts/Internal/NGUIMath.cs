...
static public class NGUIMath
{
	...
	{
		return CalculateRelativeWidgetBounds(trans, trans, false, null );
	}

	static public Bounds CalculateRelativeWidgetBounds (Transform trans, string skipName )
	{
		return CalculateRelativeWidgetBounds(trans, trans, false, skipName );
	}
	...
	{
		return CalculateRelativeWidgetBounds(trans, trans, considerInactive, null );
	}

	static public Bounds CalculateRelativeWidgetBounds (Transform trans, bool considerInactive, string skipName )
	{
		return CalculateRelativeWidgetBounds(trans, trans, considerInactive, skipName );
	}
	...
	{
		return CalculateRelativeWidgetBounds(relativeTo, content, false, null );
	}
	...
	{	
		return CalculateRelativeWidgetBounds( relativeTo, content, considerInactive, null );
	}	

	static public Bounds CalculateRelativeWidgetBounds (Transform relativeTo, Transform content, bool considerInactive, bool considerChildren )
	{	
		return CalculateRelativeWidgetBounds( relativeTo, content, considerInactive, null, considerChildren );
	}	

	static public Bounds CalculateRelativeWidgetBounds (Transform relativeTo, Transform content, bool considerInactive, string skipName, bool considerChildren = true)
	{
		if (content != null && relativeTo != null)
		{
			...
			CalculateRelativeWidgetBounds(content, considerInactive, true, skipName, ref toLocal, ref min, ref max, ref isSet, considerChildren);
	...
	[System.Diagnostics.DebuggerHidden]
	[System.Diagnostics.DebuggerStepThrough]
	static void CalculateRelativeWidgetBounds (Transform content, bool considerInactive, bool isRoot, string skipName,
		ref Matrix4x4 toLocal, ref Vector3 vMin, ref Vector3 vMax, ref bool isSet, bool considerChildren)
	{
		...
		else // No panel present
		{
			...
			if( string.IsNullOrEmpty(skipName) || !content.name.EndsWith(skipName)) {
				...
				for (int i = 0, imax = content.childCount; i < imax; ++i)
					CalculateRelativeWidgetBounds(content.GetChild(i), considerInactive, false, skipName, ref toLocal, ref vMin, ref vMax, ref isSet, true);				
			}
		}
	}
