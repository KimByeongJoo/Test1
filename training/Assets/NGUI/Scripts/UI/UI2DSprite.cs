...
public class UI2DSprite : UIBasicSprite
{
	...
	[HideInInspector][SerializeField] float mConvertPortrait = -1f;
	// [HideInInspector][SerializeField] float mUVScale = 1f;
	...
	public override Vector4 drawingDimensions
	{
		get
		{
			...
			if (mSprite != null && mType != UISprite.Type.Tiled && mConvertPortrait < 0f)
			{
	...
	public float ConvertPortrait {
		get {
			return mConvertPortrait;
		}

		set {
			if( mConvertPortrait != value ) {
				mConvertPortrait = value;
				MarkAsChanged();
			}
		}
	}
...
#if UNITY_EDITOR
		if( mSprite != null && mSprite.packed ) {
			if( mSprite.packingMode != SpritePackingMode.Rectangle ) {
				Debug.LogError( "make sure ui 2d sprites must be rectable. " + mSprite.name );
			}
		}
#endif
		Rect rt = (mSprite != null) ? mSprite.textureRect : new Rect(0f, 0f, tex.width, tex.height);

		if( mConvertPortrait > 0f && mSprite != null ) {
			Vector2 pivot = mSprite.pivot;
			rt = new Rect( rt.x + pivot.x - mConvertPortrait, rt.y + pivot.y - mConvertPortrait, mConvertPortrait*2f, mConvertPortrait*2f );
		}

		// Vector2 expand = rt.size * ( 1f - mUVScale );

		// rt.width += expand.x;
		// rt.height += expand.y;
		// rt.center -= expand * 0.5f;
	...
	protected virtual void RefreshMaterialOption() {
		if( sprite2D != null && sprite2D.associatedAlphaSplitTexture != null ) {
			this.onRender = OnRenderETC;
		} else {
			this.onRender = null;
		}
	}

	protected void OnRenderETC( Material mat ) {
		if( sprite2D != null ) {
			const string key = "ALPHA_SPLIT_ENABLED";
			if( sprite2D.associatedAlphaSplitTexture != null ) {
				if( !mat.IsKeywordEnabled( key ) ) {
					mat.EnableKeyword( key );
				}
				
				if( sprite2D != null && sprite2D.associatedAlphaSplitTexture != null ) {
					mat.SetTexture( "_AlphaTex", sprite2D.associatedAlphaSplitTexture );
				}
			} else {
				if( mat.IsKeywordEnabled( key ) ) {
